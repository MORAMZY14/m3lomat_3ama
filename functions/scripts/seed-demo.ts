...
async function main(): Promise<void> {
  ...
  batch.set(db.collection("categoryGroups").doc("demo-main"), { name: "الفئات التجريبية", sortOrder: 0 });
  for (const [id, name, image] of categories) {
    batch.set(db.collection("categories").doc(id), { name, arabicName: name, image, groupId: "demo-main", createdAt: FieldValue.serverTimestamp() });
    const rows = bank[id];
